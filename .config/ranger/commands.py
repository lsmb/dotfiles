...
class bgs(Command):
    ...
    def execute(self):
        ...
        path = join(self.fm.thisdir.path)
        if lexists(path):
            matches = []
            wlimit = 800
            rlimit = 1.2
            reverse = False
            if self.arg(1):
                wlimit = int(self.arg(1))
            if self.arg(2):
                rlimit = float(self.arg(2))
            if self.arg(3) == '0':
                reverse = True
            for (path, _, files) in os.walk(path):
                for file in files:
                    if file.endswith(('.jpg', '.png', 'jpeg')):
                        image = os.path.abspath(os.path.join(path, file))
                        try:
                            with Image.open(image) as img:
                                width, height = img.size
                                ratio = width / height
                                #print(ratio)
                                if ratio >= rlimit and width >= wlimit and not reverse:
                                    #print(image)
                                    #matches += f"'{image}' "
                                    matches.append(image)
                                elif ratio <= rlimit and width >= wlimit and reverse:
                                    matches.append(image)
                        except:
                            print(f"Couldn't print my boi {image}")
            with open('/tmp/bgmatchesranger.txt', 'w') as out:
                for match in matches:
                    out.write(match + '\n')
            self.fm.execute_command("mpv --config-dir=$HOME/.config/mpv/mvi --playlist=/tmp/bgmatchesranger.txt")
            #self.fm.execute_command(matches)
        else:
